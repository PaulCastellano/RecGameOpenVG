...
void Game::scoreScreen()
{
	...
	if (this->colorMode == 2)
	{
		...
		//TO DO!!!
		Fill(0, 0, 0, 1);
		Text(TextWidth(s, SerifTypeface, this->windowWidth / this->settingsSize) + 2 * this->xOffset + 2 * this->windowWidth / this->settingsSize + TextWidth("X", SerifTypeface, this->windowWidth / 25) / 2, this->windowHeight - 3 * TextHeight(SerifTypeface, this->windowWidth / this->settingsSize) - 3 * this->yOffset - 10, "X", SerifTypeface, this->windowWidth / 25);
	}
	Fill(0, 0, 0, 1);
	free(s);
